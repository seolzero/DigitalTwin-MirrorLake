...
let SESSIONID = "";
const gwOptions = {
   hostname: config.gwOptions.hostname,
   port: config.gwOptions.port,
};
...
function create_flink_DO_table(DOobject) {
   ...
   // Create DO Table
   let createStreamSQL = {
      statement: `CREATE TABLE ${DOName}(\`sensorname\` STRING,\`con\` STRING, \`rowtime\` TIMESTAMP(3)) WITH ('connector' = 'kafka', 'topic' = 'DO_${DOName}','properties.bootstrap.servers' = '${config.kafkaHost}', 'format'='json')`,
   };
...
app.delete("/DigitalTwin/simulationGroup/:simulationName", async (req, res) => {
   if (req.params.simulationName) {
      ...
      if (flag) {
         ...
         deleteSinkConnector(req.params.simulationName);
         res.send({ success: 1 });
      } else {
         res.status(200).send("Unregistered simulation");
      }
...
function deleteSinkConnector(connectorName){
   var req = unirest('DELETE', `http://${config.kafkaconnectHost}/connectors/${connectorName}`)
   .end(function (res) { 
...
function deleteSink(connectorName) {
   ...
   var request = http.request(options, function (response) {
      ...
      response.on("end", function () {
         console.log("deleteSink: ", fullBody);
      });

      response.on("error", function (error) {
         console.error("deleteSink: ", error);
      });
   });
   request.end();
}
   ...
   req.on("end", async function () {
      ...
      if (req.params.simName) {
         ...
         if (flag) {
            ...
            await CreateSimulationSinkConnector(resObject);
            res.status(200).send(resObject);
         } else {
            res.status(200).send("Unregistered simulation");
         }
...
async function CreateSimulationSinkConnector(resObject) {
   ...
   var request = http.request(options, function (response) {
      ...
      response.on("end", function (chunk) {
         var body = Buffer.concat(chunks);
         console.log("sinkConnectorBody end: ", body.toString());
      });
